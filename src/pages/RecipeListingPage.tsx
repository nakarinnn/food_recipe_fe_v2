...
const RecipeListingPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTotalRecipes(sortedFoods.length);
  ...
  const currentRecipes = filteredFoods.slice(indexOfFirstRecipe, indexOfLastRecipe);
  ...
  const toggleLike = async (recipeId: string) => {
    ...
    try {
      ...
      setLikedRecipes((prev) =>
        response.data.liked ? [...prev, recipeId] : prev.filter((id) => id !== recipeId)
      );
    } catch (error) {
